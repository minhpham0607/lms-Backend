...
public interface QuizzesMapper {
    ...
    @Insert("INSERT INTO quizzes (course_id, title, description, time_limit, publish, allow_multiple_attempts) " +
            "VALUES (#{courseId}, #{title}, #{description}, #{timeLimit}, #{publish}, #{allowMultipleAttempts})")
    ...
    @Update("UPDATE quizzes SET title = #{title}, description = #{description}, " +
            "time_limit = #{timeLimit}, publish = #{publish}, allow_multiple_attempts = #{allowMultipleAttempts} WHERE quiz_id = #{quizId}")
